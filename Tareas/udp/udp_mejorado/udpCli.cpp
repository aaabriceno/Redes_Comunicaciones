...
void handle_exit(int signum) {
    if (!globalNickname.empty()) {
        ...
        vector<char> msg = {'x'};
        udp_send(&client, msg, &serverAddr);
    }
    close(client.sockfd);
    cout << "Adiós!\n";
    exit(signum);
...
bool registrarNombre() {
    ...
    while (true) {
        ...
        udp_send(&client, send, &serverAddr);

        // Esperar respuesta
        int n = udp_receive(&client, buffer, &serverAddr);
...
void readThread() {
    ...
    while (true) {
        int n = udp_receive(&client, data, &serverAddr);
        if (n <= 0) continue;
...
void enviarArchivoConVerificacion() {
    ...
    while (true) {
        ...
        udp_send(&client, datagram, &serverAddr, false);
    ...
    cout << "ℹ️  Hash SHA-256 del archivo: " << hash << "\n";
}
...
int main() {
    ...
    // Crear socket UDP
    client = udp_create_socket(0, NULL, 0);
    ...
    while (running) {
        ...
        if (input == "Y" || input == "y") {
            ...
            udp_send(&client, msg_send, &serverAddr);
        ...
        if (option == 7) {
            ...
            if (inGame && myTurn) {
                ...
                if (subOption == 1) {
                    ...
                    vector<char> msg_send(msg.begin(), msg.end());
                    udp_send(&client, msg_send, &serverAddr);
                    cout << "\n👁️ Solicitando unirse como espectador...\n";
                }
            } else {
